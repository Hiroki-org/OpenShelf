...
## 2024-05-15 - トースト通知の ARIA role 最適化
**Learning:** 動的に表示されるメッセージコンテナにおいて、エラーのような緊急度の高い通知には `role="alert"`、成功や情報などの緊急度の低い通知には `role="status"` を設定することで、スクリーンリーダーの読み上げ挙動を制御できる。これを混同すると、ユーザーの作業を不必要に中断させてしまう。
**Action:** 動的なメッセージUI（トースト、スナックバー等）を実装する際は、通知の種類（type）に応じて `alert` と `status` の role を動的に切り替えるようにする。
