...
**Action:** 今後、リスト内やコンテキストが不明瞭なボタン（特に同じテキストラベルを持つ複数のボタン）を実装・改善する際は、必ず `aria-label` に詳細な説明を含めるようにします。
...
## 2024-05-20 - Markdownエディタのキーボードアクセシビリティ改善
**Learning:** Tailwind CSS によるタブリストやクリップされたコンテナ内の要素への `focus-visible:ring-inset` の適用は、フォーカスリングがコンテナ外にはみ出て見切れるのを防ぐのに有効。また `tabpanel` 自体に `tabIndex={0}` を付与することで、プレビュー内容もキーボードでアクセスしやすくなる。
**Action:** カスタムの `role="tabpanel"` を実装する際は、インタラクティブ要素が含まれない場合でもキーボードユーザーがコンテンツ領域に移動できるよう `tabIndex={0}` を付与し、視認性の高いフォーカスリングを `focus-visible:ring-inset` とともに設定する。
