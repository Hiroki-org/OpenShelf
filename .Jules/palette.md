...
## 2026-07-31 - Accessible Form Error Messages
**Learning:** フォーム送信時やデータ取得時に発生するエラーメッセージを表示する要素に `role="alert"` を付与することで、スクリーンリーダーユーザーがフォーカスを移動することなく、エラーの発生と内容を即座に認識できるようになり、アクセシビリティが大きく向上します。
**Action:** 今後、フォームのエラーメッセージや画面上の動的なエラー表示要素を実装・改善する際は、必ずコンテナに `role="alert"` を設定します。
