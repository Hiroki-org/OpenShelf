...
## 2024-07-06 - Dynamic Message Role Management
**Learning:** 成功とエラーの両方を表示する動的なメッセージコンテナでは、メッセージの重要度に応じてスクリーンリーダーへの通知方法を変える必要があります。常に `alert` を使用すると成功メッセージでも割り込みが発生してしまい、ロールがないと更新が通知されません。
**Action:** 動的なメッセージコンテナを実装する際は、常に状態に応じて `role={type === 'error' ? 'alert' : 'status'}` のようにロールを切り替えるパターンを適用する。
