...
## 2026-08-23 - Accessible Loading States for Auto-complete Inputs
**Learning:** 動的な候補を表示するオートコンプリートのようなコンポーネントでは、検索中やローディング中の状態を示すテキスト（例：「候補を取得中...」）をスクリーンリーダーに適切に伝える必要があります。これを行うには、条件付きでレンダリングされる内容自体に `aria-live` を付与するのではなく、常にマウントされているラッパーコンテナに `aria-live="polite"` を設定し、そこへ非同期的にテキストを注入する（または表示する）必要があります。さらに、入力フィールド（`<input>`）に `aria-describedby` を使用してこのローディングテキストを関連付けることで、ユーザーに入力中のフィードバックが確実に伝わります。また、無意味な記号（`-` や `+`など）を隠すために `<span aria-hidden="true">` を使用する場合は、親のボタン要素が**常に**明示的なアクセシブルネーム（`aria-label`）を持っていることを確認し、非活性状態（`disabled`）の時だけ付与される `title` 属性などに依存してはいけません。
**Action:** 今後、非同期で取得を行うオートコンプリート入力や、検索フィールドを実装・改善する際は、常にDOMに存在するコンテナを用いた `aria-live` アプローチと `aria-describedby` による連携を適用します。また、記号を隠蔽する際は親要素の恒常的なアクセシブルネームの有無を必ずチェックします。
