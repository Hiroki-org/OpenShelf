...
## 2026-07-08 - Dynamic Message ARIA Roles
**Learning:** 成功とエラーの両方のメッセージを同じコンテナで表示する場合、エラーには `alert` ロール、成功メッセージには `status` ロールを動的に付与 (`role={messageType === 'error' ? 'alert' : 'status'}`) することで、スクリーンリーダーのユーザー体験を最適化できます。
**Action:** 今後、動的メッセージコンテナを実装する際は、メッセージの種類に応じて適切な ARIA ロールを切り替えるようにします。
