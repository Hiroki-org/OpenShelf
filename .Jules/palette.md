...
## 2026-07-23 - Markdown Editor Focus States
**Learning:** タブ付きのコンテナ要素やクリップされた領域内にあるボタン（タブリストなど）にフォーカスリングを追加する場合、通常の `ring-offset` や `outline` を使用すると、親要素の `overflow` や境界線によってリングがクリッピングされて見えなくなる場合があります。この問題は、`focus-visible:ring-inset` を併用してフォーカスリングを要素の内側に描画することで解決できます。
**Action:** 今後、コンテナ内に密接して配置されたタブやボタンのフォーカスステートを実装する際は、レイアウト崩れやクリッピングを防ぐために `focus-visible:ring-inset` を積極的に使用します。
