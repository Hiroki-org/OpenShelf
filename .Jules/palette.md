...
## 2024-03-XX - [Dynamic Toast Message Roles]
**Learning:** トースト通知のような動的に追加されるメッセージにおいて、すべての通知に `aria-live` を指定するだけでなく、エラーメッセージには `role="alert"`、それ以外の非クリティカルな成功/情報メッセージには `role="status"` を付与することで、スクリーンリーダーのユーザーにより適切な緊急度を伝えることができる。
**Action:** 動的に表示されるメッセージコンテナを実装する際は、メッセージの重要度に応じて `role` 属性を動的に切り替える処理を含めること。
