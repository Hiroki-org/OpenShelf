...
## 2026-07-03 - Dynamic Status Message Accessibility
**Learning:** 設定画面の保存完了メッセージなどの動的に表示されるフィードバックメッセージは、視覚的には気付きやすいですが、スクリーンリーダーユーザーにはDOMの変更が伝わらない可能性があります。成功とエラー両方を表示し得るコンテナに対しては、状態に応じて `role={messageType === 'error' ? 'alert' : 'status'}` と動的に割り当てるか、単純な成功メッセージには `role="status"` を指定することで、非破壊的なアナウンスを行わせることができます。
**Action:** 今後、動的なフィードバックメッセージ（トーストではないインラインメッセージ）を実装する際は、エラーには `role="alert"`、成功や通常ステータスには `role="status"` を設定し、スクリーンリーダーがユーザーのワークフローを妨げることなく通知できるようにします。
