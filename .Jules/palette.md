...
## 2025-06-25 - Settings Message Role Accessibility Improvement
**Learning:** 動的なメッセージコンテナ（設定の保存など）がエラーだけでなく、非致命的な成功メッセージも表示する場合、ARIAロール属性を条件付きで設定する（例：`role={messageType === 'error' ? 'alert' : 'status'}`）ことで、スクリーンリーダーが適切な挙動でユーザーにフィードバックを伝えることができます。
**Action:** 今後、動的なステータスメッセージやサクセスメッセージを表示するコンポーネントを実装・改善する際は、必ずメッセージの性質（成功かエラーか）に応じて適切な ARIA role を割り当てるようにします。
