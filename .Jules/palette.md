...
## 2024-07-07 - トースト通知の役割に応じた ARIA ロールの使い分け
**Learning:** トースト通知のような動的なメッセージコンテナでは、メッセージの重要度に応じて `role="alert"` （クリティカルなエラー）と `role="status"` （作業を中断しない成功・情報通知）を使い分けることで、スクリーンリーダー利用者の体験が向上する。
**Action:** 動的に表示されるメッセージコンポーネントを実装する際は、その通知の性質（エラーかそれ以外か）に基づいて適切な ARIA ロールを条件付きで設定する。
