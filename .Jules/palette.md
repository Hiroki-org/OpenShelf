...
## 2026-07-23 - Focus Visible with Ring Inset
**Learning:** タブUIや切り抜き（clip/overflow-hiddenなど）が設定されたコンテナ内でキーボードフォーカスリングを表示する際、通常の `ring-offset` や `ring` を使用すると、コンテナの端でリングが切り取られて（クリッピングされて）見えなくなってしまう問題があります。このような場合には `focus-visible:ring-inset` を併用することで、フォーカスリングが要素の内側に描画され、クリッピングを防ぎつつアクセシブルなフォーカス状態を維持できます。
**Action:** 今後、タブリスト内のタブや、境界が密接しているレイアウト内の入力要素（textareaなど）に対してフォーカススタイルを追加・改善する際は、必ず `focus-visible:ring-inset` を用いてリングが見えなくなる事態を防ぎます。
