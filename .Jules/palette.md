...
## 2024-05-18 - フォーム送信時の動的メッセージのアクシビリティ改善
**Learning:** 設定画面のプロフィール保存などで、成功・失敗のメッセージを動的に表示する際、`<p>` タグ単体ではスクリーンリーダーに変更が通知されないパターンが存在した。エラーと成功メッセージが同一のコンテナを使い回す場合は、条件分岐を用いて `role="alert"` と `role="status"` を動的に切り替える必要がある。また、これらの役割を追加した際は、テストケースも `findByText` から `findByRole("alert")` や `findByRole("status")` に変更することで、アクセシビリティツリーへの登録を保証できる。
**Action:** 動的に表示される成功・エラーメッセージのコンテナを見つけた際は、必ず適切な ARIA ロール (`alert` または `status`) が設定されているか確認し、無ければ追加する。併せてテストケースのクエリも `findByRole` に修正する。
