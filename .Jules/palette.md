...
## 2026-06-25 - Visible Focus for Keyboard Navigation
**Learning:** サイトの主要な導線となるヘッダーなどのボタンに、マウス操作時（hover）のフィードバックだけでなく、キーボード操作時（focus-visible）の明示的なフィードバック（例: `focus-visible:ring-2` など）を持たせることで、キーボードユーザーが現在どこを操作しているのかを容易に把握でき、アクセシビリティが大幅に向上します。
**Action:** 今後、グローバルなナビゲーションや主要なアクションボタンを実装・改善する際は、必ず `focus-visible` スタイルを追加し、多様な操作手段に対するアクセシビリティを確保するようにします。
