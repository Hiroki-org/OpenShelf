...
## 2026-06-21 - Use Shared Spinner Component
**Learning:** ローディング中などを示す視覚的なスピナーを表示する際、インラインでアニメーションクラスをハードコードするのではなく、既存の共有コンポーネント（例: `<Spinner />`）を再利用することが重要です。これにより、視覚的な一貫性が保たれるだけでなく、`aria-hidden="true"` などのアクセシビリティ属性が確実に適用され、スクリーンリーダーの不要な読み上げを防ぐことができます。
**Action:** 今後、非同期処理のボタンなどでローディング表示を追加または修正する際は、インラインの CSS クラスで実装するのではなく、プロジェクト内の共有 `<Spinner />` コンポーネントを使用します。
