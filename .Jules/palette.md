...
## 2026-07-03 - Accessible PDF Viewer Control Buttons
**Learning:** PDFビューアーの「前へ」「次へ」や検索の「前の一致」「次の一致」といったボタンは、ラベルテキストが短すぎてスクリーンリーダーユーザーにコンテキストが伝わりにくい場合があります。これらに明示的な `aria-label`（例: 「前のページ」「次のページ」）を追加することでアクセシビリティが向上します。また、アクセシブルな名前を変更した場合は、必ず関連する単体テスト（getByRoleのname指定など）を追従させる必要があります。
**Action:** 今後、コンテキストが不足しがちな短いテキストラベルを持つコントロールボタンを実装・改善する際は、必ず意味のある `aria-label` を設定し、関連する単体テストを修正するようにします。
