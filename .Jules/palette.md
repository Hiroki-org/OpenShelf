...
## 2024-07-06 - トースト通知の動的role属性設定とID生成
**Learning:**
- トーストコンポーネントのような動的メッセージコンテナにおいて、単一の `role` ではアクセシビリティ要件を満たせない。エラー時にはスクリーンリーダーが即座に読み上げるための `role="alert"`、成功や情報といった作業を妨げるべきではない通知には `role="status"` といった動的な切り替えが必要である。
- コンポーネント内のID生成（とくにReactなどのクライアントサイド）において `Math.random().toString(36)` のような一意性の低い乱数を利用することは避けるべきであり、標準のWeb Crypto APIである `crypto.randomUUID()` を使うべきである。
**Action:** 動的に表示される要素を設計する際は、そのメッセージの重要性に応じて適切なARIA Role（alert/status）を付与するようにする。クライアント側での一時的なID生成であっても、必ず `crypto.randomUUID()` を使用する。
