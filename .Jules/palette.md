...
## 2026-07-15 - FeedButton Focus States and Auto-Select
**Learning:** `FeedButton` などのポップオーバーを持つコンポーネントでは、キーボード操作でタブ移動する際、デフォルトでは現在のフォーカス位置が分かりにくいことがあります。Tailwind の `focus-visible:ring` クラスを用いて明示的なフォーカスリングを追加することで、アクセシビリティが向上します。また、URLを表示する唯読の `textarea` においては、`onFocus={(e) => e.target.select()}` を設定することで、クリックまたはタブ移動した瞬間にURLが全選択され、コピー操作のUXが向上します。
**Action:** 今後、ポップオーバー内のインタラクティブ要素や、コピーされることを前提とした読み取り専用テキストエリアを実装・改善する際は、必ず明示的な `focus-visible` スタイルと `onFocus` 時の自動選択機能を組み込むようにします。
