...
## 2026-06-25 - Shared Components for Accessibility Consistency
**Learning:** ローディングスピナーのように、複数の場所で利用されるUI要素において、インラインのHTML（`<span className="animate-spin..." />`など）を直接記述すると、`aria-hidden="true"`などの重要なアクセシビリティ属性が一部で欠落するリスクが高まります。共有コンポーネント（例：`<Spinner />`）を作成し、そこにアクセシビリティ要件を集約して各所で再利用することで、プロダクト全体での一貫したアクセシビリティとコードの保守性が保証されます。また、テストコードにおいても、要素の検索条件（例：`querySelector('[aria-hidden="true"].motion-safe\\:animate-spin')`）を共通化できるメリットがあります。
**Action:** 今後、視覚的・アクセシビリティ的に同一のパターンを持つUI要素（スピナー、アイコン、共通ボタンなど）を発見した場合は、インラインでの実装を避け、共有コンポーネントに抽出し、プロジェクト全体で再利用するようにリファクタリングを検討します。
