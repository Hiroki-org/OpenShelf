...
## 2026-06-22 - Error Message Accessibility with role="alert"
**Learning:** フォームのバリデーションエラーやAPIのフェッチエラーなど、動的に表示されるエラーメッセージに `role="alert"` を付与することで、エラー発生時にスクリーンリーダーが即座にユーザーに通知できるようになり、大幅にアクセシビリティが向上します。
**Action:** 今後、エラーメッセージを動的に表示する UI コンポーネントを実装・改善する際は、必ずエラーコンテナ（`<p>` や `<div>` など）に `role="alert"` を設定します。
