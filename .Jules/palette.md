...
## 2026-07-24 - ローディングスピナーのコンポーネント化
**Learning:** 様々なボタンで `animate-spin` を使用したインラインのローディングスピナーをハードコーディングすると、コードの重複が発生し、UIの一貫性が損なわれます。
**Action:** インラインの `span` スピナーを共通の `<Spinner />` コンポーネントに置き換えました。これにより、`aria-hidden="true"` や `motion-safe` などのアクセシビリティ属性が適切に設定され、アプリ全体で一貫したユーザー体験を提供できるようになります。
