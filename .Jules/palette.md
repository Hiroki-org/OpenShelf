...
## 2026-06-21 - Org Slug Validation Accessibility
**Learning:** 組織作成時の slug など、動的にバリデーションが行われる入力フィールドにおいて、バリデーションステータスの `<p>` タグに `aria-live="polite"` を付与し、かつ `<input>` 要素に `aria-describedby` で関連付けることで、ユーザーが入力した際に非同期で変化する「確認中...」や「✓ 使用可能」などの状態がスクリーンリーダーに正しく読み上げられるようになり、アクセシビリティが向上します。また、`✓` などの装飾アイコンには `aria-hidden="true"` を設定することで、スクリーンリーダーによる不要な読み上げ（例: 「チェックマーク 使用可能」などの重複）を防ぐことができます。
**Action:** 今後、動的な入力バリデーション（可用性チェックなど）を実装・改善する際は、必ず入力フィールドに `aria-describedby` を付与し、ステータス表示の要素に `aria-live="polite"` を付与するパターンを適用します。また、視覚的な装飾としてアイコンを含める場合は、スクリーンリーダーからは見えないように `aria-hidden="true"` を指定します。
